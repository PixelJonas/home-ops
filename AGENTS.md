...
- The LiteLLM deployment (`components-apps/litellm/`) runs with `STORE_MODEL_IN_DB=true` (set in `litellm-app.yaml`) — the git-tracked `litellm-config.yaml`'s `model_list` is **not** loaded into the running proxy at all; confirmed live by editing it and finding `/v1/models` unchanged. Manage models via LiteLLM's Model Management API (`POST /model/new`, `/model/update`, `/model/delete`) or the Admin UI's Models page instead. Keep `litellm-config.yaml` limited to `general_settings`/`litellm_settings`.
